...
int16_t coines_open_comm_intf(enum coines_comm_intf intf_type, void *arg)
{
    ...
    app30_eeprom_init();
    if (app30_eeprom_read(0x60, eeprom_read_buff, EEPROM_READ_LEN))
    {
        /* Check the CS pin byte value,Should not be zero,if zero read failed */
        if (eeprom_read_buff[EEPROM_CS_BYTE_INDEX] != 0)
        {
